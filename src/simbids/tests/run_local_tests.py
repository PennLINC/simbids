...
def run_command(command, env=None):
    ...
    Keep this out of the real simbids code so that devs don't need
    to install SimBIDS to run tests.
    """
...
def run_tests(test_regex, test_mark):
    ...
    mounted_code = '/usr/local/miniconda/lib/python3.10/site-packages/simbids'
    run_str = 'docker run --rm -ti '
    run_str += f'-v {local_patch}:{mounted_code} '
    run_str += '--entrypoint pytest '
    run_str += 'pennlinc/simbids:unstable '
    run_str += (
        f'{mounted_code}/simbids '
        f'--data_dir={mounted_code}/simbids/tests/test_data '
        f'--output_dir={mounted_code}/simbids/tests/pytests/out '
        f'--working_dir={mounted_code}/simbids/tests/pytests/work '
    )
    if test_regex:
