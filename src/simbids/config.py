...
#
r"""
A Python module to maintain unique, run-wide *SimBIDS* settings.

This module implements the memory structures to keep a consistent, singleton config.
Settings are passed across processes via filesystem, and a copy of the settings for
each run and subject is left under
``<fmriprep_dir>/sub-<participant_id>/log/<run_unique_id>/simbids.toml``.
Settings are stored using :abbr:`ToML (Tom's Markup Language)`.
The module has a :py:func:`~simbids.config.to_filename` function to allow writing out
the settings to hard disk in *ToML* format, which looks like:

.. literalinclude:: ../simbids/data/tests/config.toml
   :language: toml
   :name: simbids.toml
   :caption: **Example file representation of SimBIDS settings**.

This config file is used to pass the settings across processes,
using the :py:func:`~simbids.config.load` function.
...
.. code-block:: Python
    ...
    config_file = config.execution.work_dir / '.simbids.toml'
...
if not any(
    ...
    # https://github.com/nipreps/simbids/pull/2080#discussion_r409118765
    from fmriprep._warnings import logging
else:
    import logging
...
if os.getenv('IS_DOCKER_8395080871'):
    ...
    if _cgroup.exists() and 'docker' in _cgroup.read_text():
        ...
        _exec_env = 'simbids-docker' if _docker_ver else 'docker'
    del _cgroup
...
class environment(_Config):
    ...
    execution environment, nipype and *SimBIDS* versions, etc.).
...
class execution(_Config):
    ...
    notrack = False
    """Do not collect telemetry information for *SimBIDS*."""
    track_carbon = False
...
def load(filename, skip=None, init=True):
    ...
    filename : :py:class:`os.PathLike`
        TOML file containing SimBIDS configuration.
    skip : dict or None
