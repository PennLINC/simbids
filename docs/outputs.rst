...
*SimBIDS* outputs conform to the :abbr:`BIDS (brain imaging data structure)`
Derivatives specification (see `BIDS Derivatives`_, along with the
upcoming `BEP 011`_ and `BEP 012`_).
*SimBIDS* generates three broad classes of outcomes:
...
1.  **Visual QA (quality assessment) reports**:
    ...
    of processing and ensures the transparency of *SimBIDS* operation.
...
Assuming SimBIDS is invoked with::

    simbids <input_dir>/ <output_dir>/ participant [OPTIONS]
...
``dataset_description.json`` is a metadata file in which SimBIDS
...
*SimBIDS* outputs summary reports,
written to ``<output dir>/simbids/sub-<label>.html``.
These reports provide a quick way to make visual inspection of the results easy.


**********************************
Derivatives of *SimBIDS*
**********************************
...
For each :abbr:`BOLD (blood-oxygen level dependent)` run processed with *SimBIDS*,
...
*SimBIDS* outputs a set of confounds that can be used to denoise the data.
...
The confounds generated by *SimBIDS* are ICA component time series
...
See implementation on :mod:`~simbids.workflows.bold.confounds.init_bold_confs_wf`.
