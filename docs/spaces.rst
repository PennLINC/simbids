...
.. warning::
   ...
   SimBIDS currently only generates denoised data in MNI152NLin6Asym space.

#####################################################################
Defining standard and nonstandard spaces where data will be resampled
#####################################################################

The command line interface of *SimBIDS* allows resampling the preprocessed data
...
.. important::
   *SimBIDS* will reduce the amount of output spaces to just spaces listed in ``--output-spaces``,
...
that allows end-user applications such as *SimBIDS* to flexibly query and pull
template and atlas information.
In other words, *TemplateFlow* enables *SimBIDS* to access a wide range
of templates (and also custom templates, see below).
Therefore, *TemplateFlow* is central to define *SimBIDS*'s interface regarding
...
When using *SimBIDS* in a workflow that will investigate effects that span across
...
For instance, to instruct *SimBIDS* to use the MNI template brain distributed with
FSL as coordinate reference the option will read as follows: ``--output-spaces MNI152NLin6Asym``.
By default, *SimBIDS* uses ``MNI152NLin6Asym`` as spatial-standardization reference.
...
Therefore, *SimBIDS* will run nonlinear registration processes against the template
T1w image corresponding to all the standard spaces supplied with the argument
``--output-spaces``.
By default, *SimBIDS* will resample the preprocessed data on those spaces (labeling the
...
To make your custom templates visible by *SimBIDS*, and usable via
...
* ``fsnative``: similarly to the ``anat`` space for volumetric references,
  including the ``fsnative`` space will instruct *SimBIDS* to sample the
...
In other words, running *SimBIDS* with ``--output-spaces MNI152NLin6Asym:res-2
